...
export class EventosController {
  ...
  @Post(':id/reservar')
  reserveSpots(
    @Body() reservarLugar: ReservarLugarRequest,
    @Param('id') eventoId: string,
  ) {
    return this.eventsService.reserveSpot({
      eventId: eventoId,
      spots: reservarLugar.lugares,
      ticket_kind: reservarLugar.tipo_ingresso === 'inteira' ? TicketKind.full : TicketKind.half,
      email: reservarLugar.email,
    });
  }
}
